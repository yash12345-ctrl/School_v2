      ...
      const colorUtils = (function (colorsArr: number[]) {
        ...
        return {
          ...
          getColorAt: (ratio: number, out: Color = new Color()) => {
            // Use ratio to deterministically pick a discrete color from the palette without interpolation
            const idx = Math.floor(ratio * baseColors.length);
            const clampedIdx = Math.max(0, Math.min(baseColors.length - 1, idx));
            const c = colorObjects[clampedIdx];
            out.r = c.r;
            out.g = c.g;
            out.b = c.b;
            return out;
